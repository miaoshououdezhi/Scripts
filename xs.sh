...
if [ -z "${BASH_VERSION:-}" ]; then
    ...
    if [ -f /etc/alpine-release ] && command -v apk >/dev/null 2>&1; then
        ...
        cp /etc/apk/repositories "/etc/apk/repositories.xs.bak.$(date +%s 2>/dev/null || echo 0)" 2>/dev/null || true
...
# 基础路径定义
export SCRIPT_VERSION="16.0-xs-dual-core"
...
XS_DIR="/usr/local/etc/xs"
XS_ACTIVE_CORE_FILE="${XS_DIR}/active-core"
...
_manage_service() {
    ...
    if [[ "$action" == "restart" || "$action" == "start" ]]; then
        if [ "${XS_SWITCHING_CORE:-0}" != "1" ]; then
            _manage_xray_core_service "stop" >/dev/null 2>&1 || true
        fi
...
_manage_xray_core_service() {
    local action="$1"
    [ -z "$INIT_SYSTEM" ] && _detect_init_system
    case "$INIT_SYSTEM" in
        systemd)
            if [ "$action" = "status" ]; then systemctl status xray --no-pager -l; return; fi
            _with_timeout 30 systemctl "$action" xray
            ;;
        openrc)
            if [ "$action" = "status" ]; then rc-service xray status; return; fi
            _with_timeout 30 rc-service xray "$action"
            ;;
...
_mark_active_core() {
    local core="$1"
    mkdir -p "$XS_DIR" 2>/dev/null || true
    echo "$core" > "$XS_ACTIVE_CORE_FILE" 2>/dev/null || true
}

_get_active_core() {
    if [ -s "$XS_ACTIVE_CORE_FILE" ]; then
        cat "$XS_ACTIVE_CORE_FILE" 2>/dev/null
        return
    fi
    if [ "$INIT_SYSTEM" = "systemd" ]; then
        systemctl is-active --quiet sing-box 2>/dev/null && { echo "sing-box"; return; }
        systemctl is-active --quiet xray 2>/dev/null && { echo "xray"; return; }
    elif [ "$INIT_SYSTEM" = "openrc" ]; then
        rc-service sing-box status 2>/dev/null | grep -q "started" && { echo "sing-box"; return; }
        rc-service xray status 2>/dev/null | grep -q "started" && { echo "xray"; return; }
    fi
    echo "none"
}
...
_check_xray_config() {
    ...
    local xray_cfg="$XRAY_CONFIG_FILE"
    [ -x "$xray_bin" ] && [ -s "$xray_cfg" ] || return 1
    "$xray_bin" run -test -config "$xray_cfg" >/dev/null 2>&1 || "$xray_bin" test -config "$xray_cfg" >/dev/null 2>&1
}

_switch_core() {
    local target="$1"
    export XS_SWITCHING_CORE=1
    case "$target" in
        sing-box)
            _require_singbox || { unset XS_SWITCHING_CORE; return 1; }
            if [ -x "$SINGBOX_BIN" ] && [ -s "$CONFIG_FILE" ]; then
                "$SINGBOX_BIN" check -c "$CONFIG_FILE" >/dev/null 2>&1 || { _error "sing-box 配置检查失败，已取消切换。"; unset XS_SWITCHING_CORE; return 1; }
            fi
            _info "正在切换到 sing-box：先停止 Xray，再启动 sing-box。"
            _manage_xray_core_service "stop" >/dev/null 2>&1 || true
            _manage_service "restart" || { unset XS_SWITCHING_CORE; return 1; }
            _mark_active_core "sing-box"
            _success "当前运行内核：sing-box"
            ;;
        xray)
            [ -x "$XRAY_BIN" ] || { _error "Xray 核心未安装。"; unset XS_SWITCHING_CORE; return 1; }
            [ -s "$XRAY_CONFIG_FILE" ] || { _error "Xray 配置不存在。"; unset XS_SWITCHING_CORE; return 1; }
            _check_xray_config || { _error "Xray 配置检查失败，已取消切换。"; unset XS_SWITCHING_CORE; return 1; }
            _info "正在切换到 Xray：先停止 sing-box，再启动 Xray。"
            _manage_service "stop" >/dev/null 2>&1 || true
            _manage_xray_core_service "restart" || { unset XS_SWITCHING_CORE; return 1; }
            _mark_active_core "xray"
            _success "当前运行内核：Xray"
            ;;
        stop)
            _manage_service "stop" >/dev/null 2>&1 || true
            _manage_xray_core_service "stop" >/dev/null 2>&1 || true
            _mark_active_core "none"
            _success "两个核心均已停止。"
            ;;
        *) _error "未知内核: $target"; unset XS_SWITCHING_CORE; return 1 ;;
    esac
    unset XS_SWITCHING_CORE
}

_convert_singbox_to_xray() {
    command -v jq >/dev/null 2>&1 || { _error "缺少 jq，无法转换配置。"; return 1; }
    [ -s "$CONFIG_FILE" ] || { _error "未找到 sing-box 配置: $CONFIG_FILE"; return 1; }
    mkdir -p "$XRAY_DIR"
    [ -s "$XRAY_CONFIG_FILE" ] && cp -f "$XRAY_CONFIG_FILE" "${XRAY_CONFIG_FILE}.bak.$(date +%s)" 2>/dev/null || true

    local unsupported
    unsupported=$(jq '[.inbounds[]? | select((.tag // "") | contains("-hop-") | not) | select((.type == "vless" or .type == "shadowsocks" or .type == "trojan") | not)] | length' "$CONFIG_FILE")
    jq '{
      log: {loglevel: "warning"},
      inbounds: [
        .inbounds[]? |
        select((.tag // "") | contains("-hop-") | not) |
        if .type == "vless" then
          {
            tag: .tag,
            listen: (.listen // "::"),
            port: .listen_port,
            protocol: "vless",
            settings: {decryption: "none", clients: [(.users[0] | {id: .uuid, flow: (.flow // "")})]},
            streamSettings: (
              if (.tls.reality.enabled // false) then
                {network: (.transport.type // "tcp"), security: "reality", realitySettings: {show: false, dest: ((.tls.reality.handshake.server // .tls.server_name // "www.amd.com") + ":443"), serverNames: [(.tls.reality.handshake.server // .tls.server_name // "www.amd.com")], privateKey: .tls.reality.private_key, shortIds: (.tls.reality.short_id // [])}}
              elif (.tls.enabled // false) then
                {network: (.transport.type // "tcp"), security: "tls", tlsSettings: {serverName: (.tls.server_name // ""), certificates: [{certificateFile: (.tls.certificate_path // ""), keyFile: (.tls.key_path // "")}]}}
              else
          ...
          {tag: .tag, listen: (.listen // "::"), port: .listen_port, protocol: "shadowsocks", settings: {method: .method, password: .password, network: "tcp,udp"}}
        elif .type == "trojan" then
          {
            ...
            port: .listen_port,
            protocol: "trojan",
            settings: {clients: [{password: .users[0].password}]},
            streamSettings: ({network: (.transport.type // "tcp")} + (if (.tls.enabled // false) then {security: "tls", tlsSettings: {serverName: (.tls.server_name // ""), certificates: [{certificateFile: (.tls.certificate_path // ""), keyFile: (.tls.key_path // "")}]}} else {} end) + (if (.transport.type // "") == "ws" then {wsSettings: {path: (.transport.path // "/")}} else {} end))
          }
        else empty end
      ],
      outbounds: [{protocol: "freedom", tag: "direct"}],
      routing: {rules: []}
    }' "$CONFIG_FILE" > "$XRAY_CONFIG_FILE" || return 1
    [ -s "$XRAY_METADATA_FILE" ] || echo '{}' > "$XRAY_METADATA_FILE"
    [ "${XS_SKIP_SERVICE_CREATE:-0}" = "1" ] || _create_xray_service_from_main 2>/dev/null || true
    _check_xray_config || { _error "转换后的 Xray 配置检查失败，已保留备份。"; return 1; }
    _success "已将 sing-box 节点转换为 Xray 配置。"
    [ "$unsupported" -gt 0 ] 2>/dev/null && _warn "有 ${unsupported} 个 sing-box 专属/暂不兼容节点未转换。"
    return 0
}

_convert_xray_to_singbox() {
    command -v jq >/dev/null 2>&1 || { _error "缺少 jq，无法转换配置。"; return 1; }
    local xray_cfg="$XRAY_CONFIG_FILE"
    [ -s "$xray_cfg" ] || { _error "未找到 Xray 配置: $xray_cfg"; return 1; }
    mkdir -p "$SINGBOX_DIR"
    [ -s "$CONFIG_FILE" ] && cp -f "$CONFIG_FILE" "${CONFIG_FILE}.bak.$(date +%s)" 2>/dev/null || true

    local unsupported
    unsupported=$(jq '[.inbounds[]? | select((.protocol == "vless" or .protocol == "shadowsocks" or .protocol == "trojan") | not)] | length' "$xray_cfg")
    jq '{
      log: {level: "warn", timestamp: false},
      dns: {servers: [{tag: "dns-cloudflare", address: "https://1.1.1.1/dns-query", detour: "direct"}], strategy: "ipv4_only"},
      inbounds: [
        .inbounds[]? |
        if .protocol == "vless" then
          {
            type: "vless",
            tag: (.tag // ("xray-vless-in-" + (.port|tostring))),
            listen: (.listen // "::"),
            listen_port: .port,
            users: [(.settings.clients[0] | {uuid: .id, flow: (.flow // "")})],
            tls: (
              if (.streamSettings.security // "") == "reality" then
                {enabled: true, server_name: (.streamSettings.realitySettings.serverNames[0] // "www.amd.com"), reality: {enabled: true, handshake: {server: (.streamSettings.realitySettings.serverNames[0] // "www.amd.com"), server_port: 443}, private_key: .streamSettings.realitySettings.privateKey, short_id: (.streamSettings.realitySettings.shortIds // [])}}
              elif (.streamSettings.security // "") == "tls" then
                {enabled: true, server_name: (.streamSettings.tlsSettings.serverName // ""), certificate_path: (.streamSettings.tlsSettings.certificates[0].certificateFile // ""), key_path: (.streamSettings.tlsSettings.certificates[0].keyFile // "")}
              else {enabled: false} end
            )
          } + (if (.streamSettings.network // "") == "ws" then {transport: {type: "ws", path: (.streamSettings.wsSettings.path // "/")}} else {} end)
        elif .protocol == "shadowsocks" then
          {type: "shadowsocks", tag: (.tag // ("xray-ss-in-" + (.port|tostring))), listen: (.listen // "::"), listen_port: .port, method: .settings.method, password: .settings.password}
        elif .protocol == "trojan" then
          {
            type: "trojan",
            tag: (.tag // ("xray-trojan-in-" + (.port|tostring))),
            listen: (.listen // "::"),
            listen_port: .port,
            users: [{password: .settings.clients[0].password}],
            tls: (if (.streamSettings.security // "") == "tls" then {enabled: true, server_name: (.streamSettings.tlsSettings.serverName // ""), certificate_path: (.streamSettings.tlsSettings.certificates[0].certificateFile // ""), key_path: (.streamSettings.tlsSettings.certificates[0].keyFile // "")} else {enabled: false} end)
          } + (if (.streamSettings.network // "") == "ws" then {transport: {type: "ws", path: (.streamSettings.wsSettings.path // "/")}} else {} end)
        else empty end
      ],
      outbounds: [{type: "direct", tag: "direct"}],
      route: {rules: [], final: "direct"}
    }' "$xray_cfg" > "$CONFIG_FILE" || return 1
    [ -s "$METADATA_FILE" ] || echo '{}' > "$METADATA_FILE"

    jq -r '.inbounds[]? | select(.type == "vless" and (.tls.reality.enabled // false)) | [.tag, (.tls.reality.private_key // ""), (.tls.reality.short_id[0] // "")] | @tsv' "$CONFIG_FILE" | while IFS=$'\t' read -r tag private_key sid; do
        local public_key
        public_key=$(_xray_public_key_from_private "$private_key")
        [ -n "$public_key" ] && _atomic_modify_json "$METADATA_FILE" ". + {\"$tag\": {\"publicKey\": \"$public_key\", \"shortId\": \"$sid\"}}" 2>/dev/null || true
    done

    _rebuild_clash_from_singbox_config 2>/dev/null || _warn "已转换服务端配置，但客户端 YAML 重建不完整。"
    if [ -x "$SINGBOX_BIN" ]; then
        "$SINGBOX_BIN" check -c "$CONFIG_FILE" >/dev/null 2>&1 || { _error "转换后的 sing-box 配置检查失败，已保留备份。"; return 1; }
    fi
    [ "${XS_SKIP_SERVICE_CREATE:-0}" = "1" ] || _create_service_files 2>/dev/null || true
    _success "已将 Xray 节点转换为 sing-box 配置。"
    [ "$unsupported" -gt 0 ] 2>/dev/null && _warn "有 ${unsupported} 个 Xray 专属/暂不兼容节点未转换。"
    return 0
}
...
_conversion_menu() {
    while true; do
        clear
        echo -e "${CYAN}  ╭──────── 配置互转 ────────╮${NC}"
        echo -e "  ${GREEN}[1]${NC} sing-box -> Xray"
        echo -e "  ${GREEN}[2]${NC} Xray -> sing-box"
        echo -e "  ${YELLOW}[0]${NC} 返回"
        echo -e "${CYAN}  ╰──────────────────────────╯${NC}"
        read -p "  请选择 [0-2]: " choice
        case "$choice" in
            1) _convert_singbox_to_xray ;;
            2) _convert_xray_to_singbox ;;
            0) return ;;
            *) _error "无效输入。" ;;
        esac
        echo
...
_alpine_prepare_repositories() {
    ...
    local force="${1:-}"
    local state_file="/tmp/.xs_apk_repo_ready"
    if [ "$force" != "force" ] && [ -f "$state_file" ]; then
        return 0
    fi

    local backup="/etc/apk/repositories.xs.bak.$(date +%s 2>/dev/null || echo 0)"
    cp /etc/apk/repositories "$backup" 2>/dev/null || true

    local base seen_file="/tmp/.xs_apk_repo_seen.$$"
    : > "$seen_file"
...
_pkg_install() {
    ...
    if command -v apk >/dev/null 2>&1; then
        ...
        _with_timeout "${SB_APK_ADD_TIMEOUT:-120}" apk add --no-progress $pkgs || {
            ...
            rm -f /tmp/.xs_apk_repo_ready 2>/dev/null || true
...
_xray_public_key_from_private() {
    local private_key="$1"
    [ -n "$private_key" ] || return 1
    if [ -x "$XRAY_BIN" ]; then
        "$XRAY_BIN" x25519 -i "$private_key" 2>/dev/null | awk -F': ' '/Public key|PublicKey|Public/{print $2; exit}'
    fi
}

_rebuild_clash_from_singbox_config() {
    [ -s "$CONFIG_FILE" ] || return 1
    command -v jq >/dev/null 2>&1 || return 1
    command -v "$YQ_BINARY" >/dev/null 2>&1 || return 1
    _ensure_clash_yaml_skeleton
    [ -z "$server_ip" ] && server_ip=$(_get_public_ip)
    [ -z "$server_ip" ] && server_ip="127.0.0.1"

    ${YQ_BINARY} eval '.proxies = [] | (.proxy-groups[] | select(.name == "节点选择") | .proxies) = []' -i "$CLASH_YAML_FILE" 2>/dev/null || return 1

    jq -c '.inbounds[]? | select((.tag // "") | contains("-hop-") | not)' "$CONFIG_FILE" | while IFS= read -r node; do
        local type tag port name proxy_json
        type=$(echo "$node" | jq -r '.type // empty')
        tag=$(echo "$node" | jq -r '.tag // empty')
        port=$(echo "$node" | jq -r '.listen_port // empty')
        [ -n "$type" ] && [ -n "$tag" ] && [ -n "$port" ] || continue
        name="XS-${type}-${port}"

        case "$type" in
            vless)
                local uuid flow sn is_reality pbk sid
                uuid=$(echo "$node" | jq -r '.users[0].uuid // empty')
                flow=$(echo "$node" | jq -r '.users[0].flow // ""')
                sn=$(echo "$node" | jq -r '.tls.server_name // .tls.reality.handshake.server // env.DEFAULT_SNI')
                is_reality=$(echo "$node" | jq -r '.tls.reality.enabled // false')
                if [ "$is_reality" = "true" ]; then
                    pbk=$(jq -r --arg t "$tag" '.[$t].publicKey // empty' "$METADATA_FILE" 2>/dev/null)
                    sid=$(echo "$node" | jq -r '.tls.reality.short_id[0] // ""')
                    [ -z "$sid" ] && sid=$(jq -r --arg t "$tag" '.[$t].shortId // empty' "$METADATA_FILE" 2>/dev/null)
                    proxy_json=$(jq -n --arg n "$name" --arg s "$server_ip" --arg p "$port" --arg u "$uuid" --arg sn "$sn" --arg pbk "$pbk" --arg sid "$sid" --arg flow "${flow:-xtls-rprx-vision}" \
            ...
            shadowsocks)
                ...
                method=$(echo "$node" | jq -r '.method // empty')
                password=$(echo "$node" | jq -r '.password // empty')
                proxy_json=$(jq -n --arg n "$name" --arg s "$server_ip" --arg p "$port" --arg m "$method" --arg pw "$password" \
                    '{"name":$n,"type":"ss","server":$s,"port":($p|tonumber),"cipher":$m,"password":$pw}')
                _add_node_to_yaml "$proxy_json"
                ;;
            trojan)
                local password sn ws_path
                password=$(echo "$node" | jq -r '.users[0].password // empty')
                sn=$(echo "$node" | jq -r '.tls.server_name // env.DEFAULT_SNI')
                ws_path=$(echo "$node" | jq -r '.transport.path // ""')
                proxy_json=$(jq -n --arg n "$name" --arg s "$server_ip" --arg p "$port" --arg pw "$password" --arg sn "$sn" --arg wsp "$ws_path" \
                    '{"name":$n,"type":"trojan","server":$s,"port":($p|tonumber),"password":$pw,"udp":true,"sni":$sn,"skip-cert-verify":true} | if $wsp != "" then .network="ws" | .["ws-opts"]={"path":$wsp,"headers":{"Host":$sn}} else . end')
                _add_node_to_yaml "$proxy_json"
                ;;
        esac
    done
...
# 安装命令行呼出入口：执行 xs 即可再次打开脚本。
_install_cli_shortcut() {
    local shortcut="/usr/local/bin/xs"
    ...
    if [ -f "$SELF_SCRIPT_PATH" ] && [ -s "$SELF_SCRIPT_PATH" ]; then
        ...
        chmod +x "$shortcut" 2>/dev/null || true
        _success "快捷命令已安装：xs"
        _info "后续可直接在终端输入 xs 呼出本脚本菜单。"
        return 0
    fi

    _warn "无法从当前运行路径复制脚本，快捷命令 xs 暂未安装。建议先保存脚本文件后再运行。"
    return 1
}
...
export XRAY_CONFIG_FILE="${XRAY_CONFIG_FILE:-${XRAY_DIR}/config.json}"
export XRAY_METADATA_FILE="${XRAY_METADATA_FILE:-${XRAY_DIR}/metadata.json}"
...
_uninstall() {
    ...
    echo -e "  ${RED}-${NC} 快捷命令: /usr/local/bin/xs"
    ...
    rm -f "/usr/local/bin/sb" "/usr/local/bin/xs"
...
_update_script() {
    ...
    if _download_file "$SCRIPT_UPDATE_URL" "$temp_script_path" "主脚本更新包" "$SB_DOWNLOAD_TIMEOUT"; then
        ...
        _success "主脚本 (X-ray && sing-box Manager) 更新成功！"
    else
...
_do_update_xray() {
    ...
    if [ "$is_first_install" = true ]; then
        ...
        _info "Xray 首次安装完成。正在切换到 Xray 内核..."
        _switch_core "xray"
    else
        # 已安装：重启服务
        _create_xray_service_from_main
...
_main_menu() {
    while true; do
        ...
        echo -e "${CYAN}"
        echo "  ╔═══════════════════════════════════════╗"
        echo "  ║         X-ray && sing-box 管理 v${SCRIPT_VERSION}         ║"
        echo "  ╚═══════════════════════════════════════╝"
        echo -e "${NC}"
        echo -e "  ${BLUE}平台官网：Shlii.io${NC}"
        echo -e "  ${CYAN}快捷呼出：${YELLOW}xs${NC}"
        echo -e "  ${CYAN}安装完成后，终端输入 ${YELLOW}xs${CYAN} 可再次打开本脚本${NC}"
        echo ""
        echo -e "  ${RED}禁止测速${NC}"
        echo -e "  ${RED}禁止滥用${NC}"
...
_print_xs_header() {
    ...
    echo '        X-ray && sing-box Manager  '
    echo -e "${NC}"
    echo -e "  ${CYAN}╔════════════════════════════════════════════════╗${NC}"
    echo -e "  ${CYAN}║${NC}  X-ray && sing-box 管理脚本  v${SCRIPT_VERSION}  ${CYAN}║${NC}"
    echo -e "  ${CYAN}╚════════════════════════════════════════════════╝${NC}"
    echo -e "  ${BLUE}平台官网：Shlii.io${NC}"
    echo -e "  ${CYAN}快捷呼出：${YELLOW}xs${NC}"
    echo -e "  ${CYAN}安装完成后，终端输入 ${YELLOW}xs${CYAN} 可再次打开本脚本${NC}"
...
_print_xs_status() {
    ...
    local sb_version="" sb_status="${RED}○ 未安装${NC}" sb_nodes="0"
    if [ -f "$SINGBOX_BIN" ]; then
        sb_version=" v$($SINGBOX_BIN version 2>/dev/null | head -n1 | awk '{print $3}')"
        sb_nodes=$(jq '[.inbounds[]? | select((.tag // "") | contains("-hop-") | not)] | length' "$CONFIG_FILE" 2>/dev/null || echo 0)
        if [ "$INIT_SYSTEM" = "systemd" ] && systemctl is-active --quiet sing-box 2>/dev/null; then
            sb_status="${GREEN}● 运行中${NC} (${sb_nodes}节点)"
        elif [ "$INIT_SYSTEM" = "openrc" ] && rc-service sing-box status 2>/dev/null | grep -q "started"; then
            sb_status="${GREEN}● 运行中${NC} (${sb_nodes}节点)"
        else
            sb_status="${YELLOW}○ 已停止${NC} (${sb_nodes}节点)"
        fi
    fi

    local xray_version="" xray_status="${RED}○ 未安装${NC}" xray_nodes="0"
    if [ -f "/usr/local/bin/xray" ]; then
        xray_version=" v$(/usr/local/bin/xray version 2>/dev/null | head -1 | awk '{print $2}')"
        xray_nodes=$(jq '[.inbounds[]?] | length' /usr/local/etc/xray/config.json 2>/dev/null || echo 0)
        if [ "$INIT_SYSTEM" = "systemd" ] && systemctl is-active --quiet xray 2>/dev/null; then
            xray_status="${GREEN}● 运行中${NC} (${xray_nodes}节点)"
        elif [ "$INIT_SYSTEM" = "openrc" ] && rc-service xray status 2>/dev/null | grep -q "started"; then
            xray_status="${GREEN}● 运行中${NC} (${xray_nodes}节点)"
        else
            xray_status="${YELLOW}○ 已停止${NC} (${xray_nodes}节点)"
        fi
    fi

    echo ""
    echo -e "  系统：${CYAN}${os_info}${NC}  |  服务框架：${CYAN}${INIT_SYSTEM}${NC}  |  当前内核：${YELLOW}$(_get_active_core)${NC}"
    echo -e "  sing-box${CYAN}${sb_version}${NC}: ${sb_status}"
    echo -e "  Xray${CYAN}${xray_version}${NC}: ${xray_status}"
    echo -e "  ${RED}提示：禁止滥用，禁止机场接入。${NC}"
    echo ""
}

_xs_service_menu() {
    while true; do
        clear; _print_xs_header; _print_xs_status
        echo -e "  ${CYAN}【服务控制】${NC}"
        echo -e "    ${GREEN}[1]${NC} 重启当前内核        ${GREEN}[2]${NC} 停止当前内核"
        echo -e "    ${GREEN}[3]${NC} 查看 sing-box 状态   ${GREEN}[4]${NC} 查看 Xray 状态"
        echo -e "    ${GREEN}[5]${NC} 查看 sing-box 日志"
        echo -e "    ${YELLOW}[0]${NC} 返回主菜单"
        read -p "  请选择 [0-5]: " choice
        local active=$(_get_active_core)
        case "$choice" in
            1) [ "$active" = "xray" ] && _manage_xray_core_service restart || _require_singbox && _manage_service restart ;;
            2) [ "$active" = "xray" ] && _manage_xray_core_service stop || _require_singbox && _manage_service stop ;;
            3) _require_singbox && _manage_service status ;;
            4) _manage_xray_core_service status ;;
            5) _require_singbox && _view_log ;;
            0) return ;;
            *) _error "无效输入。" ;;
        esac
...
_xs_node_menu() {
    while true; do
        clear; _print_xs_header; _print_xs_status
        echo -e "  ${CYAN}【节点管理】${NC}"
        echo -e "    ${GREEN}[1]${NC} sing-box 添加节点     ${GREEN}[2]${NC} sing-box 查看链接"
        echo -e "    ${GREEN}[3]${NC} sing-box 删除节点     ${GREEN}[4]${NC} sing-box 修改端口"
        echo -e "    ${GREEN}[5]${NC} Xray 节点管理         ${GREEN}[6]${NC} Argo 隧道节点"
        echo -e "    ${YELLOW}[0]${NC} 返回主菜单"
        read -p "  请选择 [0-6]: " choice
        case "$choice" in
            1) _require_singbox && _show_add_node_menu ;;
            2) _require_singbox && _view_nodes ;;
            3) _require_singbox && _delete_node ;;
            4) _require_singbox && _modify_port ;;
            5) _xray_features ;;
            6) _require_singbox && _argo_menu ;;
            0) return ;;
            *) _error "无效输入。" ;;
        esac
        echo; read -n 1 -s -r -p "按任意键继续..."
...
_xs_core_menu() {
    while true; do
        clear; _print_xs_header; _print_xs_status
        echo -e "  ${CYAN}【核心管理】${NC}"
        echo -e "    ${GREEN}[1]${NC} 安装/更新 sing-box     ${GREEN}[2]${NC} 安装/更新 Xray"
        echo -e "    ${GREEN}[3]${NC} 切换到 sing-box        ${GREEN}[4]${NC} 切换到 Xray"
        echo -e "    ${GREEN}[5]${NC} 节点配置互转          ${GREEN}[6]${NC} 停止全部核心"
        echo -e "    ${YELLOW}[0]${NC} 返回主菜单"
        read -p "  请选择 [0-6]: " choice
        case "$choice" in
            1) _install_or_update_singbox ;;
            2) _install_or_update_xray ;;
            3) _switch_core "sing-box" ;;
            4) _switch_core "xray" ;;
            5) _conversion_menu ;;
            6) _switch_core "stop" ;;
            0) return ;;
...
_xs_tools_menu() {
    while true; do
        clear; _print_xs_header; _print_xs_status
        echo -e "  ${CYAN}【工具维护】${NC}"
        echo -e "    ${GREEN}[1]${NC} 检查 sing-box 配置   ${GREEN}[2]${NC} 同步系统时间"
        echo -e "    ${GREEN}[3]${NC} 定时重启设置        ${GREEN}[4]${NC} 更新脚本"
        echo -e "    ${GREEN}[5]${NC} 落地/中转/导入      ${RED}[6]${NC} 卸载脚本"
        echo -e "    ${YELLOW}[0]${NC} 返回主菜单"
        read -p "  请选择 [0-6]: " choice
        case "$choice" in
            1) _require_singbox && _check_config ;;
            2) _sync_system_time ;;
            3) _require_singbox && _scheduled_restart_menu ;;
            4) _update_script ;;
            5) _require_singbox && _advanced_features ;;
            6) _uninstall ;;
            0) return ;;
            *) _error "无效输入。" ;;
        esac
...
_main_menu() {
    while true; do
        ...
        _print_xs_status
        echo -e "  ${CYAN}【主菜单】${NC}"
        echo -e "    ${GREEN}[1]${NC} 节点管理      ${GREEN}[2]${NC} 核心管理"
        echo -e "    ${GREEN}[3]${NC} 服务控制      ${GREEN}[4]${NC} 工具维护"
        echo -e "    ${YELLOW}[0]${NC} 退出脚本"
        echo ""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        check-os|doctor)
            ...
            for c in bash curl wget jq openssl tar timeout update-ca-certificates ip ss rc-service xs; do command -v "$c" >/dev/null 2>&1 && echo "OK: $c" || echo "MISS: $c"; done
